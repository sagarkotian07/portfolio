...
// ---- desktop: scripted completion ----
{
  ...
  await page.goto(base, { waitUntil: 'networkidle' }); await page.waitForTimeout(3500);
  ...
  const result = await page.evaluate(() => {
    ...
    for (let i = 0; i < 60 * 150; i++) {
      game.tick(1 / 60);
      if (i % 30 === 0) { const d = game.debug(); trace.push({ t: i / 60, x: Math.round(d.x / 40 * 10) / 10, y: Math.round(d.y / 40 * 10) / 10, vx: Math.round(d.vx), g: d.grounded ? 1 : 0, form: d.form[0], l: d.lives, r: d.rings }); }
      if (game.state === 'won' || game.state === 'over') break;
    }
    const d = game.debug(); game.draw();
    return { ...d, chunk: window.__bounce.game.level.chunkOffsets.findIndex((o, i, a) => d.x / 40 >= o && (i === a.length - 1 || d.x / 40 < a[i + 1])), trace };
  });
  await page.screenshot({ path: `${out}/game-final.png` });
  console.log('desktop run:', JSON.stringify({ state: result.state, lives: result.lives, rings: result.rings, xTiles: Math.round(result.x / 40), chunk: result.chunk, form: result.form }));
  ...
  // a few mid-level frames for the eye
  for (const [name, tx, ty] of [['meadow', 30, 13], ['gate', 92, 13], ['platforms', 118, 13], ['fans', 196, 12], ['finish', 236, 6]]) {
    await page.evaluate(([x, y]) => { const { game } = window.__bounce; document.getElementById('game-start').hidden = true; document.getElementById('game-end').hidden = true; game.start(); game.teleport(x * 40, y * 40); for (let i = 0; i < 40; i++) game.tick(1 / 60); game.draw(); }, [tx, ty]);
