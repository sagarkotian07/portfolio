// Drives the game deterministically through tick(), follows the waypoints, and checks the level can be won.
...
// ---- desktop: scripted completion ----
{
  ...
  const result = await page.evaluate(() => {
    ...
    return { ...d, trace, time: game.time };
  });
  await page.screenshot({ path: `${out}/game-final.png` });
  console.log('desktop run:', JSON.stringify({ state: result.state, deaths: result.deaths, eggs: result.eggs, x: Math.round(result.x / D * 10) / 10, y: Math.round(result.y / D * 10) / 10, wp: result.wp, seconds: Math.round(result.time) }));
  ...
  for (const [name, tx, ty] of [['start', 0, 0], ['plank', 73, 7.1], ['hill', 118, 6.2], ['floor', 199, 33.5], ['machine', 216, 7.5]]) {
...
// ---- phone: touch buttons move and jump; page does not scroll while playing ----
{
  ...
  const phoneRoute = await page.evaluate(() => { const { game, waypoints } = window.__bounce; game.start(); game.autopilot(waypoints); for (let i = 0; i < 60 * 200; i++) { game.tick(1 / 60); if (game.state === 'won') break; } const d = game.debug(); return { state: game.state, deaths: d.deaths, eggs: d.eggs, time: Math.round(game.time) }; });
