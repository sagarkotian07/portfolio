...
export function mountGame(opts: { onPlay(): void; onStop(): void }) {
  ...
  q('#game-title').textContent = copy.title;
  q('#game-controls').textContent = copy.controls; whenTouch(() => { q('#game-controls').textContent = copy.touch; });

  const showEnd = (title: string, line: string) => { q('#game-end-title').textContent = title; q('#game-end-line').textContent = line; end.hidden = false; leave.hidden = true; deactivate(); };

  const g = new Game(canvas, {
    onHud() { /* the HUD is drawn on the canvas */ },
    onWin(e, total, time) { setTimeout(() => showEnd(copy.win(e, total, fmt(time)), copy.winLine), 1400); },
